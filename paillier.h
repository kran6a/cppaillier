...
private:
    [[nodiscard]] inline int_t randomR() const;
public:
    static constexpr unsigned short const DEFAULT_KEYSIZE = 3072;
    unsigned short keysize = 0;
    //int_t MAX_SAFE_INTEGER;
    int_t n;
    int_t g;
    int_t _n2;
    explicit PaillierPublicKey(const std::string &n);
    explicit PaillierPublicKey(const int_t &n);
    PaillierPublicKey() = default;
    [[nodiscard]] int_t encrypt(const int_t &cleartext, const int_t &r = int_t{0}) const;
    [[nodiscard]] std::pair<const int_t, const int_t> rEncrypt(const int_t &cleartext) const;
    [[nodiscard]] static int_t add(const int_t &enc_n, const int_t &enc_m, const PaillierPublicKey &pk);
    [[nodiscard]] inline int_t add(const int_t &enc_n, const int_t &enc_m) const;
    [[nodiscard]] inline int_t raw_add(const int_t &enc_n, const int_t &raw_m) const;
    [[nodiscard]] inline static int_t sub(const int_t &enc_n, const int_t &enc_m, const PaillierPublicKey &pk);
    [[nodiscard]] inline int_t sub(const int_t &enc_n, const int_t &enc_m) const;
    [[nodiscard]] inline static int_t mul(const int_t &enc_n, const int_t &raw_m, const PaillierPublicKey &pk);
    [[nodiscard]] inline int_t mul(const int_t &enc_n, const int_t &raw_m) const;
...
private:
    ...
    [[nodiscard]] inline int_t crt(const int_t &n, const int_t &m) const;
public:
    ...
    [[nodiscard]] inline PaillierPublicKey getPublicKey() const;
    ...
    [[nodiscard]] inline static int_t L_function(const int_t &n, const int_t &p);
    [[nodiscard]] inline static int_t H_function(const int_t &n, const int_t &n2, const int_t &g);
    [[nodiscard]] inline int_t computeR(const int_t &ciphertext) const;
    [[nodiscard]] json to_json() const;
    [[nodiscard]] inline bool ZKPCorrectDecryption(const int_t &ciphtext, const int_t &VerifierRandomValue) const;
